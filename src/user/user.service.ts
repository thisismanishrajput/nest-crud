...
export class UserService {
  ...
  async createUser(
    ...
    try {
      return await this.userModel.create({
        ...
        email: this.normalizeEmail(registerUserDto.email),
        isVerified: false,
        otp,
        otpExpiresAt,
        otpAttempts: 0,
  ...
  async updateOtp(
    ...
    return await this.userModel
      .findOneAndUpdate({ email: this.normalizeEmail(email) }, updatePayload, {
  ...
  async markEmailVerified(email: string) {
    return await this.userModel
      .findOneAndUpdate(
        ...
        { isVerified: true, otp: null, otpExpiresAt: null, otpAttempts: 0 },
        { returnDocument: 'after' },
      )
      .exec();
  }
  ...
  async loginUser(loginUserDto: LoginUserDto) {
    try {
      return await this.userModel
        .findOne({ email: this.normalizeEmail(loginUserDto.email) })
        .select('+password')
        .exec();
    } catch (err: unknown) {
      console.log(err);
